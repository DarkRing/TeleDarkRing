...
if msg.to.type == 'chat' then
  if is_admin1(msg) not is_support(msg.from.id) then-- Admin only
 ...
 --[[Experimental
  ...
  if msg.to.id and data[tostring(msg.to.id)] then
    ...
    if matches[1] == 'setname' or "نام گروه" and is_momod(msg) then
    ...
    if matches[1] == 'setphoto' or "عکس گروه" and is_momod(msg) then
    ...
    if matches[1] == 'about' or "درباره" then
...
--Begin chat settings
    if matches[1] == 'lock' or "قفل" then
      local target = msg.to.id
		if matches[2] == 'name' or "نام" then
		...
		if matches[2] == 'flood' or "اسپم" then
		...
		if matches[2] == 'bots' or "ربات" then
			savelog(msg.to.id, name_log.." ["..msg.from.id.."] locked bots ")
			return lock_group_bots(msg, data, target)
		end
		if matches[2] == 'leave' or "خروج" then
    ...
    if matches[1] == 'unlock' or "بازکردن" then
		local target = msg.to.id
		if matches[2] == 'name' or "نام" then
		...
		if matches[2] == 'photo' or "عکس" then
  ...
  if matches[1] == 'mute' and is_owner(msg) then
	...
	--Begin chat muteuser
		if matches[1] == "muteuser" or "خفه کردن" and is_momod(msg) then
		local chat_id = msg.to.id
		...
		local user_id = ""
			...
			elseif matches[1] == "muteuser" or "خفه کردن" and string.match(matches[2], '^%d+$') then
			...
			elseif matches[1] == "muteuser" or "خفه کردن" and not string.match(matches[2], '^%d+$') then
				local receiver = get_receiver(msg)
				local get_cmd = "mute_user"
				local username = matches[2]
				local username = string.gsub(matches[2], '@', '')
...
if msg.to.type == 'chat' then
    if matches[1] == 'newlink' or "لينک جديد" and not is_realm(msg) then
      if not is_momod(msg) then
        return "For moderators only!"
      end
...
if msg.to.type == 'chat' then
    if matches[1] == 'clean' or "حذف" then
      ...
      if matches[2] == 'member' or "اعضا" then
        if not is_owner(msg) then
...
if msg.to.type == 'chat' then
    ...
    if matches[1] == 'help' or "راهنما" then
    ...
    if matches[1] == 'kickinactive' or "اخراج غيرفعال" then
