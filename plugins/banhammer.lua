...
local support_id = msg.from.id
 if matches[1]:lower() == 'id' or 'ايدي' and msg.to.type == "chat" or msg.to.type == "user" then
    ...
    elseif matches[1]:lower() == 'id' or "ايدي" then
      local name = user_print_name(msg.from)
      savelog(msg.to.id, name.." ["..msg.from.id.."] used /id ")
      return "Group ID for " ..string.gsub(msg.to.print_name, "_", " ").. ":\n\n"..msg.to.id
    end
  end
  if matches[1]:lower() == 'kickme' or "خروج" and msg.to.type == "chat" then-- /kickme
  ...
  if matches[1]:lower() == "banlist" or "ليست بن" then -- Ban list !
    local chat_id = msg.to.id
  ...
  if matches[1]:lower() == 'ban' or "بن" then-- /ban
  ...
  if matches[1]:lower() == 'unban' or "حذف بن" then -- /unban
    if type(msg.reply_id)~="nil" and is_momod(msg) then
...
if matches[1]:lower() == 'kick' or "اخراج" then
    if type(msg.reply_id)~="nil" and is_momod(msg) then
      if is_admin1(msg) then
  ...
  if matches[1]:lower() == 'banall' or "سور بن" and is_admin1(msg) then -- Global ban
  ...
  if matches[1]:lower() == 'unbanall' or "حذف سوپربن" then -- Global unban
    local user_id = matches[2]
    local chat_id = msg.to.id
  ...
  if matches[1]:lower() == "gbanlist" or "ليست سوپربن" then -- Global ban list
    return banall_list()
  end
end
