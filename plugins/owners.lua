...
local function run(msg, matches)
  if msg.to.type == 'user' then
    ...
    if matches[2] == 'unban' or "حذف بن" then
    if tonumber(matches[3]) == tonumber(our_id) then return false end
      local chat_id = matches[1]
      if not is_owner2(msg.from.id, chat_id) then
        return "You are not the owner of this group"
    ...
    if matches[2] == 'kick' or "اخراج" then
      local chat_id = matches[1]
      if not is_owner2(msg.from.id, chat_id) then
        return "You are not the owner of this group"
      end
    ...
    if matches[2] == 'clean' or "حذف" then
      ...
      if matches[3] == 'rules' or "قوانين" then
        if not is_owner2(msg.from.id, chat_id) then
          return "You are not the owner of this group"
        end
        local data_cat = 'rules'
      ...
      if matches[3] == 'about' or "درباره" then
        if not is_owner2(msg.from.id, chat_id) then
          return "You are not the owner of this group"
        end
        local data_cat = 'description'
    ...
    if matches[2] == "setflood" or "حساسيت اسپم" then
      if not is_owner2(msg.from.id, chat_id) then
        return "You are not the owner of this group"
      end
    ...
    if matches[2] == 'unlock' or "بازکردن" then
      if not is_owner2(msg.from.id, chat_id) then
        return "You are not the owner of this group"
      end
      local target = matches[1]
      ...
      if matches[3] == 'member' or "ورود" then
	  ...
	  if matches[3] == 'links' or "لينک" and group_type == "SuperGroup" then
		savelog(matches[1], name.." ["..msg.from.id.."] unlocked links ")
		return unlock_group_links(msg, data, target)
	  end
	  if matches[3] == 'spam' or "اسپم" and group_type == "SuperGroup" then
	  ...
	  if matches[3] == 'sticker' or "استيکر" and group_type == "SuperGroup" then
		savelog(matches[1], name.." ["..msg.from.id.."] unlocked sticker")
		return unlock_group_sticker(msg, data, target)
	  end
	  if matches[3] == 'contacts' or "مخاطبين" and group_type == "SuperGroup" then
	...
	if matches[2] == 'new' or "جديد" then
      if matches[3] == 'link' or "لينک" then
	   local group_type = data[tostring(matches[1])]['group_type']
        if not is_owner2(msg.from.id, chat_id) then
          return "You are not the owner of this group"
        end
    ...
    if matches[1] == 'changeabout' or "ثبت درباره" and matches[2] then
	...
	if matches[1] == 'viewsettings' or "تنظيمات" and data[tostring(matches[2])]['settings'] then
	    if not is_owner2(msg.from.id, matches[2]) then
          return "You are not the owner of this group"
        end
		local target = matches[2]
    ...
    if matches[1] == 'changerules' or "ثبت قوانين" and is_owner2(msg.from.id, matches[2]) then
    ...
    if matches[1] == 'changename' or "ثبت اسم" and is_owner2(msg.from.id, matches[2]) then
