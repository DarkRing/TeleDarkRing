...
function run(msg, matches)
    ...
    local data = load_data(_config.moderation.data)
    local receiver = get_receiver(msg)
			if matches[1] == 'setabout' or "ثبت درباره" and matches[2] == 'group' and is_realm(msg) then
				local target = matches[3]
				local about = matches[4]
				return set_description(msg, data, target, about)
			end
			if matches[1] == 'setabout' or "ثبت درباره" and matches[2] == 'sgroup'and is_realm(msg) then
			...
			if matches[1] == 'setrules' or "ثبت قوانين" then
				rules = matches[3]
			...
			if matches[1] == 'lock' or "قفل" then
				...
				if matches[3] == 'member' or "اعضا" then
					return lock_group_member(msg, data, target)
				end
				if matches[3] == 'photo' or "عکس" then
					return lock_group_photo(msg, data, target)
				end
				if matches[3] == 'flood' or "فلود" then
		...
		if matches[1] == 'settings'or "تنظيمات" and matches[2] == 'group' or "گروه" and data[tostring(matches[3])]['settings'] then
			local target = matches[3]
			text = show_group_settingsmod(msg, target)
			return text.."\nID: "..target.."\n"
		end
		if  matches[1] == 'settings' or "تنظيمات" and matches[2] == 'sgroup' or "سوپرگروه" and data[tostring(matches[3])]['settings'] then
			local target = matches[3]
			text = show_supergroup_settingsmod(msg, target)
			return text.."\nID: "..target.."\n"
		end

		if matches[1] == 'setname' or "ثبت اسم" and is_realm(msg) then
		...
		if matches[1] == 'setgpname' or "ثبت اسم" and is_admin1(msg) then
    	...
    	if matches[1] == 'help' or "راهنما" and is_realm(msg) then
		...
		if matches[1] == 'type' or "نوع" then
             local group_type = get_group_type(msg)
			return group_type
		end
		if matches[1] == 'list' or "ليست" then
			if matches[2] == 'admins' or "ادمين" then
		...
		if matches[1] == 'list' or "ليست" and matches[2] == 'groups' or "گروه ها" then
		...
		if matches[1] == 'list' or "ليست" and matches[2] == 'realms' or "ريلم ها" then
			if msg.to.type == 'chat' or msg.to.type == 'channel' then
				realms_list(msg)
