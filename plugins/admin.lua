...
local function run(msg,matches)
    ...
    if matches[1] == "setbotphoto" or "عکس ربات" then
    	redis:set("bot:photo", "waiting")
    	return 'Please send me bot photo now'
    end
    if matches[1] == "markread" or "تيک دوم" then
    	if matches[2] == "on" or "روشن" then
    		redis:set("bot:markread", "on")
    		return "Mark read > on"
    	end
    	if matches[2] == "off" or "خاموش" then
    		redis:del("bot:markread")
    		return "Mark read > off"
    	end
    	return
    end
    if matches[1] == "pm" or "پيام" then
    ...
    if matches[1] == "import" or "ورود" then--join by group link
    	local hash = parsed_url(matches[2])
    	import_chat_link(hash,ok_cb,false)
    end
    if matches[1] == "contactlist" or "مخاطبين" then
    ...
    if matches[1] == "delcontact" or "حذف مخاطب" then
	    if not is_sudo(msg) then-- Sudo only
    		return
    	end
    ...
    if matches[1] == "addcontact" or "افزودن مخاطب" and is_sudo(msg) then
    phone = matches[2]
 ...
 if matches[1] == "sendcontact" or "ارسال مخاطب" and is_sudo(msg) then
    phone = matches[2]
    first_name = matches[3]
    last_name = matches[4]
    send_contact(get_receiver(msg), phone, first_name, last_name, ok_cb, false)
end
 if matches[1] == "mycontact" or "شماره من" and is_sudo(msg) then
    ...
    if matches[1] == "dialoglist" or "ليست گفتگوها" then
      get_dialog_list(get_dialog_list_callback, {target = msg.from.id})
      return "I've sent a group dialog list with both json and text format to your private messages"
    end
    if matches[1] == "whois" or "کيه" then
      user_info("user#id"..matches[2],user_info_callback,{msg=msg})
    end
    if matches[1] == "sync_gbans" or "آپديت سوپربن" then
    	if not is_sudo(msg) then-- Sudo only
    		return
    	end
	...
	if matches[1] == 'updateid' or "آپديت ايدي" then
		local data = load_data(_config.moderation.data)
	...
	if matches[1] == 'addlog' or "افزودن گزارش" and not matches[2] then
	...
	if matches[1] == 'remlog' or "حذف گزارشات" and not matches[2] then
